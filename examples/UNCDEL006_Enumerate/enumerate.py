...
from deli.enumeration.enumerator import ReactionTree, Enumerator
...
reactions = [
    ReactionStep(
        "1",
        Reaction("[#6:1]-C(=O)-O.[15N:2]>>[15N:2]-C(=O)-[#6:1]"),
        [BBSetReactant("DEL006_A"), StaticReactant(Chem.MolFromSmiles("[15N]"))],
    ),
    ReactionStep(
        "2",
        Reaction(
            "[#8]=[#6](-[#8]-[#6]-[#6]1-[#6]2:[#6]:[#6]:[#6]:[#6]:[#6]:2-[#6]2:[#6]-1:[#6]:[#6]:[#6]:[#6]:2)-[#7:1][*:2]>>[#7:1]([H])[*:2]"
        ),
        [BBSetReactant("product_1")],
    ),
    ReactionStep(
        "3",
        Reaction("[Nh:2].[#6:3](=O)-O >>[#6:3](=O)-[N:2]"),
        [
            BBSetReactant("product_2"),
            StaticReactant(Chem.MolFromSmiles("OC(C1=CC=C(F)C(N(=O)=O)=C1)=O")),
        ],
    ),
    ReactionStep(
        "4",
        Reaction(
            "[#6:0]1(-[*:1]):[#6]:[#6]:[#6:2](-[#9]):[#6:4](-[#7+](-[#8-])=[#8]):[#6]:1.[NH2:6][*:3]>>[#6:0]1(-[*:1]):[#6]:[#6]:[#6:2](-[NH:6][*:3]):[#6:4](-[#7+](-[#8-])=[#8]):[#6]:1"
        ),
        [BBSetReactant("product_3"), BBSetReactant("DEL006_B")],
    ),
    ReactionStep(
        "5",
        Reaction(
            "[#6:0]1(-[*:1]):[#6]:[#6]:[#6:2](-[#7][*:3]):[#6:4](-[#7+](-[#8-])=[#8]):[#6]:1.[CX3H1:6](=O)[*:5]>>[#6:0]1(-[*:1]):[#6]:[#6]:[#6:2]2-[#7]([*:3])-[#6:6](-[*:5])=[#7]-[#6:4]:2:[#6]:1"
        ),
        [BBSetReactant("product_4"), BBSetReactant("DEL006_C")],
    ),
]

# 3. Create Enumerator
workflow = ReactionTree(reactions)
enumerator = Enumerator([bba_set, bbb_set, bbc_set], workflow)

# 4. Enumerators create RDKit Mols directly
mol = enumerator.enumerate_by_bb_ids(["A10", "B47", "C60"])

# 5. Attach it to a library object to associate library metadata
lib = CombinatorialLibrary("DEL006", [bba_set, bbb_set, bbc_set], enumerator=enumerator)
