...
def analyze(config):
    ...
    def validate_flags(config):
        """Check if all enabled flags can be executed with current dependencies."""
        if "flags" not in config:
            return True
        flags = config["flags"]
        if flags.get("gnn_classifier", False):
            from deli.analysis.cube_factory import check_torch_available
            if not check_torch_available():
                print("ERROR: gnn_classifier flag is enabled but PyTorch is not available")
                return False
        return True

    config = load_config(config)
    if not validate_flags(config):
        print("Analysis aborted due to missing dependencies")
        return
    ...
    from deli.analysis import create_deli_cube

    cube = create_deli_cube(
        df, id_col, indexes, control_cols, int(config["general"].get("lib_size", 0)), raw_indexes
    )
