...
class PolyO:
    def __init__(self, data, indexes, lib_size, feature_mode="disynthon"):
        self.data = data
    ...
    def calculate_polyOraw(self):
        ...
        for exp_name, columns in tqdm(self.indexes.items(), desc="Experiments"):
            row_sums = self.data[columns].sum(axis=1)
            total_reads = row_sums.sum()
        ...
        for exp_name in tqdm(self.indexes.keys(), desc="Calculating Products per Disynthon"):
            total_reads = sum(
                self.data[columns].sum().sum() for columns in self.indexes.values()
            )
    ...
    def calculate_Cread(self):
        ...
        for feature in features:
            ...
            total_reads = 0
            for columns in self.indexes.values():
    ...
    def poly_o_base(self):
        ...
        total_reads = sum(self.data[columns].sum().sum() for columns in self.indexes.values())
        S_bar = total_reads / self.lib_size
